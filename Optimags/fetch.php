<?php
	...
	if ( isset( $_POST[ "view" ] ) ) {
		...
		if ( $_POST[ "view" ] != '' ) {
			$update_query = "UPDATE notif SET S_P=1 WHERE S_P=0 and id!='Optima Gs' AND komentar LIKE '%diperbaharui%' OR S_P=0 and id!='Optima Gs'";
			mysql_query( $update_query );
		}
		$querys = "SELECT * FROM notif WHERE id!= 'Optima Gs' ORDER BY waktu DESC, no DESC";
		...
		if ( mysql_num_rows( $results ) > 0 ) {
			while ( $row = mysql_fetch_array( $results ) ) {
				...
				$output .= '
				<a href="Detail.php?ni=' . $row[ "no_ccann" ] . '" style="color: #000000;Font-family:Times New Roman;text-decoration:none;">
				<li>
		...
		$query_1 = "SELECT * from notif where S_P=0 and id!='Optima Gs' AND komentar LIKE '%diperbaharui%' OR S_P=0 and id!='Optima Gs'";
