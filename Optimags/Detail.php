...
<form id="form1" name="form1" method="post">
	<table width="100%" height="178" border="1" class="zebra-table">
		<thead>
			<?php
			$cek = mysql_num_rows( mysql_query( 'SELECT ccan.*, optima.* FROM ccan LEFT JOIN optima ON ccan.no=optima.no_ccan WHERE optima.no_ccan = "' . $id . '" AND ccan.stts NOT LIKE "%Close 2%"' ) );
			if ( $cek > 0 && $stts != 'Close 2') {
			?>
			<tr>
			...
			<?php
			}else if ($cek < 2 && $stts != 'Close 2' ){
			?>
			<tr>
				<td colspan="14" width="113"><a type="button" class="btn btn-primary btn-sm center-block" href="Edit_optima_gs.php?ni=<?php echo $no;?>" title="tambah">Update Order</a>
				</td>
			</tr>
			<?php
			}else{
	...
	<table border="1" id="tabelku" width="30%" align="center" class="zebra-tables">
		...
		<tbody>
			<?php
			$query_D_C = mysql_query( "select * from vendor where no_ccanw = $no " )or die( mysql_error() );
			while ( $datas = mysql_fetch_array( $query_D_C ) ) {
				?>
			<tr>
				<td>
					<?php echo $datas['time']; ?>
