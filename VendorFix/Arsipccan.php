<?php
	...
	$tanggal_awal_CC = ubahTanggalaw( $rawaw );
	...
	$tanggal_akhir_CC = ubahTanggalak( $rawwk );
?>
<html>
	<title>Telkom</title>
...
<body>
	...
	<form id="form2" name="form2" method="postform" action="Arsipccan.php">
		<h5>Masukan Tanggal:</h5>
		<table width="70%" border="0">
			<tbody>
				<tr>
					<td width="19%">Dari Tanggal</td>
					<td width="25%"><input name="dateaw" type="date" autofocus required="required" id="dateaw" value ="<?php echo $tanggal_awal_CC; ?>">
					</td>
					<td width="20%">Sampai Tanggal</td>
					<td width="21%"><input name="dateak" type="date" required="required" id="dateak" value ="<?php echo $tanggal_akhir_CC; ?>" >
					</td>
		...
		<table width="347" border="0" class="center-block">
			<tbody>
				<tr>
					...
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td width="90"><a href="Arsipsemuaccan.php" class="btn btn-sm btn-default no-print">Lihat Semua Order</a>
					</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
	...
	<?php
		if ( isset( $_REQUEST[ 'cari' ] ) ) {
			if ( empty( $tanggal_awal_CC ) || empty( $tanggal_akhir_CC ) ) {
			?>
			<script language="JavaScript">
				alert( 'Tanggal Awal dan Tanggal Akhir Harap di Isi!' );
				document.location = 'Arsipccan.php';
			</script>
			<?php
				}else if ( $tanggal_akhir_CC < $tanggal_awal_CC ) {
			?>
			<Script language="javaScript">
				alert( 'Tanggal tidak valid' );
				document.localName = "Arsipccan.php"
			</Script>
			<?php
				} else {
			?>
			...
			<b>
				<?php echo $tanggal_awal_CC?>
			</b>s/d
			<b>
				<?php echo $tanggal_akhir_CC?>
			</b>
			<?php
			}
		?>
		<form id="form1" name="form1" method="post">
			<?php
				$query_CC = mysql_query( "SELECT ccan.*, optima.*, vendor.* FROM ccan LEFT JOIN optima ON ccan.no=optima.no_ccan LEFT JOIN vendor ON ccan.no=vendor.no_ccanw WHERE ccan.tgl BETWEEN '$tanggal_awal_CC' AND '$tanggal_akhir_CC' AND ccan.stts LIKE '%Close 2%' AND optima.vdr= '$idven' GROUP BY ccan.no ORDER BY ccan.tgl ASC, ccan.no ASC" )or die( mysql_error() );
